...
test('login rápido aparece quando habilitado em desenvolvimento',async()=>{const anterior=process.env.DEV_QUICK_LOGIN;process.env.DEV_QUICK_LOGIN='true';const resposta=await fetch(base+'/entrar');const html=await resposta.text();assert.match(html,/Entrar como aluno 1/);assert.match(html,/Entrar como aluno 2/);assert.match(html,/Entrar como professor/);if(anterior===undefined)delete process.env.DEV_QUICK_LOGIN;else process.env.DEV_QUICK_LOGIN=anterior;});
