...
export default class TccController {
  ...
  add = async (req, res) => {
    ...
    const tcc = await Tcc.create({
      ...
      ano:req.body.ano, area:req.body.area, status,
      ...
      historico:[{ status, observacao:status === 'Enviado' ? 'Trabalho enviado para avaliação.' : 'Rascunho criado.', usuario:req.session.usuario.id }]
    });
  ...
  openEdt = async (req, res) => {
    ...
    const usuario = req.session.usuario;
    if (!ehDono(tcc.alunoResponsavel, usuario) && !podeModerar(usuario)) throw Object.assign(new Error('Acesso negado.'), { status:403 });
    const [cursos, turmas, professores] = await Promise.all([Curso.find({ ativo:true }), Turma.find({ ativa:true }).populate('curso'), Usuario.find({ perfil:'professor', aprovado:true, ativo:true })]);
    res.render(this.caminhoBase + 'edt', { title:'Editar TCC', tcc, cursos, turmas, professores, editavel:ehDono(tcc.alunoResponsavel, usuario) && ['Rascunho','Correções solicitadas'].includes(tcc.status) });
  };
  ...
  edt = async (req, res) => {
    ...
    Object.assign(tcc, { titulo:req.body.titulo, resumo:req.body.resumo, palavrasChave:lista(req.body.palavrasChave), autores:lista(req.body.autores), orientador:req.body.orientador, curso:req.body.curso, turma:req.body.turma, ano:req.body.ano, area:req.body.area });
    ...
    if (req.body.acao === 'enviar') {
      ...
      tcc.historico.push({ status:tcc.status, observacao:'Nova versão enviada para avaliação.', usuario:req.session.usuario.id });
    }
  ...
  revisar = async (req, res) => {
    ...
    const permitidos = req.session.usuario.perfil === 'administrador' ? ['Aguardando revisão','Correções solicitadas','Aprovado','Publicado','Arquivado'] : ['Aguardando revisão','Correções solicitadas','Aprovado'];
    if (!permitidos.includes(status)) throw new Error('Mudança de status inválida.');
    ...
    tcc.status = status;
    if (status === 'Publicado') tcc.publicadoEm = new Date();
    tcc.historico.push({ status, observacao:req.body.observacao, usuario:req.session.usuario.id });
  ...
  catalogo = async (req, res) => {
    ...
    if (q) { const busca = new RegExp(regexSegura(q), 'i'); filtro.$or = [{titulo:busca},{resumo:busca},{palavrasChave:busca},{autores:busca}]; }
