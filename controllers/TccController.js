...
async function validarCursoTurma(curso,turma,ativos=true){
  const [cursoValido,turmaValida]=await Promise.all([
    ...
    Turma.exists({_id:turma,curso,...(ativos?{ativa:true}:{})})
  ]);
  if(!cursoValido||!turmaValida)throw falha('O curso e a turma selecionados não formam um vínculo válido.');
}

async function validarOrientador(orientador){
  if(!await Usuario.exists({_id:orientador,perfil:'professor',aprovado:true,ativo:true}))throw falha('Selecione um professor orientador ativo e aprovado.');
}
...
async function opcoesFormulario(req,tcc=null){
  ...
  let filtroCurso={ativo:true};let filtroTurma={ativa:true};
  if(vinculoAluno){
    const curso=tcc?.curso||conta.curso;const turma=tcc?.turma||conta.turma;
    if(!curso||!turma)throw falha('O administrador precisa vincular sua conta a um curso e uma turma antes do envio de TCC.',403);
    if(!tcc)await validarCursoTurma(curso,turma,true);
    filtroCurso={_id:curso};filtroTurma={_id:turma,curso};
  }
  const [cursos,turmas,professores]=await Promise.all([Curso.find(filtroCurso),Turma.find(filtroTurma).populate('curso'),Usuario.find({perfil:'professor',aprovado:true,ativo:true})]);
...
export default class TccController {
  ...
  edt = async (req, res) => {
    ...
    const conta=await Usuario.findById(req.session.usuario.id).select('perfil');
    const curso=conta?.perfil==='aluno'?tcc.curso:req.body.curso;const turma=conta?.perfil==='aluno'?tcc.turma:req.body.turma;
    await Promise.all([validarCursoTurma(curso,turma,false),validarOrientador(req.body.orientador)]);
    Object.assign(tcc, { titulo:req.body.titulo, resumo:req.body.resumo, palavrasChave:lista(req.body.palavrasChave), autores:lista(req.body.autores), orientador:req.body.orientador, curso, turma, ano:req.body.ano, area:req.body.area, ocultarAutores:req.body.ocultarAutores === 'true' });
