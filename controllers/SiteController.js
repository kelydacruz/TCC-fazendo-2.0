// Funcionalidade exclusiva do AcervoTCC: página pública com catálogo e indicadores do acervo.
    ...
    constructor() {
        this.home = async(req, res)=>{
            ...
            const [
                ...
                Tcc.distinct('ano', { status: 'Publicado' }),
                Tcc.distinct('area', { status: 'Publicado' }),
                Tcc.countDocuments({ status: 'Publicado' }),
                Tcc.aggregate([
        ...
        this.sobre = async(req, res)=>{
            const config = bancoDisponivel()
                ? await Configuracao.findOne({ chave: 'geral' })
                : null;
        ...
        this.privacidade = async(req, res)=>{
            const config = bancoDisponivel()
                ? await Configuracao.findOne({ chave: 'geral' })
                : null;
            res.render('institucional/privacidade', {
