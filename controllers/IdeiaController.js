...
export default class IdeiaController {
  ...
  list = async (req,res) => {
    if(!bancoDisponivel())return res.render(this.caminhoBase+'lst',{title:'Banco de Ideias',ideias:[],cursos:[],areas:[],statusIdeia:STATUS_IDEIA,query:req.query});
  ...
  detalhes = async (req,res) => { if(!bancoDisponivel())return res.status(404).render('404',{title:'Ideia não encontrada'});const ideia=await Ideia.findById(req.params.id).populate('curso autor tccVinculado'); if(!ideia)return res.status(404).render('404',{title:'Ideia não encontrada'}); const [comentarios,favorito]=await Promise.all([Comentario.find({ideia:ideia.id,oculto:false}).populate('autor').sort({createdAt:1}),req.session.usuario?Usuario.exists({_id:req.session.usuario.id,favoritosIdeia:ideia.id}):false]); res.render(this.caminhoBase+'detalhes',{title:ideia.titulo,ideia,comentarios,favorito:Boolean(favorito)}); };
